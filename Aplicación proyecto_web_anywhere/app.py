...
def cargar_datos(ruta_geojson, ruta_predicciones):
    ...
    predicciones = pd.read_csv(ruta_predicciones, usecols= ["DPA_DESPAR","DPA_DESCAN","DPA_DESPRO","riesgo_predicho", "probabilidad_riesgo_alto"])
    ...
    for feature in geojson["features"]:
        ...
        if key in pred.index:
            props["riesgo"] = pred.loc[key, "riesgo_predicho"]
            props["probabilidad_riesgo_alto"] = pred.loc[key, "probabilidad_riesgo_alto"]
...
def crear_mapa(geojson):
    ...
    #Luego creamos capa por nivel de provincias
    provincias = sorted({
    ...
    for provincia in provincias:
        geojson_provincia = {
            ...
            "features": [
                feature
                for feature in geojson["features"]
                if feature["properties"]["ADM1_ES"] == provincia
            ]
        }

        #Agregamos las capas por provincia
        folium.GeoJson(
            geojson_provincia,
            name=f"Provincia: {provincia}",
            style_function=color_riesgo,
